...
function ActiveCarsShiftInfoUpdate() {
    const { id } = useParams()
    ...
    const validate = () => {
        ...
        if (!shiftInfo.supportId)
            newErrors.supportId = "Помощник обязателен"
        ...
        if (!shiftInfo.parkName)
            newErrors.parkName = "Парк обязателен"
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            setIsLoading(true)
            await activeCarsShiftsWebClient.update(shiftInfo.id, {
                adminId: shiftInfo.adminId,
                supportId: shiftInfo.supportId,
                firstTicketNumber: shiftInfo.firstTicketNumber,
                parkName: shiftInfo.parkName,
        ...
        <div>
            <form className="d-flex flex-column gap-2" onSubmit={handleUpdate}>
                ...
                <div>
                    ...
                    <Select className={`form-control p-0 ${errors.supportId ? "is-invalid pe-5" : ""}`}
                        options={userOptions}
                        value={userOptions.find(opt => opt.value === shiftInfo.supportId)}
                        onChange={(selected) => {
                            setShiftInfo({ ...shiftInfo, supportId: selected.value })
                            setErrors({ ...errors, supportId: "" })
                        }}
                        classNamePrefix="custom-select"
                        placeholder="Выберите помощника..."
                    />
                    {errors.supportId &&
                        <div className=" invalid-feedback">{errors.supportId}</div>
                    }
                </div>
                <div>
                    <label>Парк:</label>
                    <Select className={`form-control p-0 ${errors.parkName ? "is-invalid pe-5" : ""}`}
                        options={Const.parkNames}
                        value={Const.parkNames.find(opt => opt.value === shiftInfo.parkName)}
                        onChange={(selected) => {
                            setShiftInfo({ ...shiftInfo, parkName: selected.value })
                            setErrors({ ...errors, parkName: "" })
                        }}
                        classNamePrefix="custom-select"
                        placeholder="Выберите парк..."
                    />
                    {errors.parkName &&
                        <div className=" invalid-feedback">{errors.parkName}</div>
                    }
                </div>
                <div>
...
export default ActiveCarsShiftInfoUpdate