...
function ActiveFoodShiftInfoUpdate() {
    const { id } = useParams()
    ...
    const validate = () => {
        ...
        if (!shiftInfo.sugarAmount)
            newErrors.sugarAmount = "Количество сахара обязательно"
        else if (shiftInfo.sugarAmount < 0.1)
            newErrors.sugarAmount = "Количество сахара обязательно"
        if (!shiftInfo.colorSugarAmount)
            newErrors.colorSugarAmount = "Количество цветного сахара обязательно"
        else if (shiftInfo.colorSugarAmount < 0.1)
            newErrors.colorSugarAmount = "Количество цветного сахара обязательно"
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            ...
            await activeFoodShiftsWebClient.update(shiftInfo.id, {
                ...
                colorSugarAmount: shiftInfo.colorSugarAmount,
        ...
        <div>
            <form className="d-flex flex-column gap-2" onSubmit={handleUpdate}>
                ...
                <div>
                    ...
                    <input className={`form-control ${errors.sugarAmount ? "is-invalid" : ""}`}
                        type="number"
                        min={0.1} max={999} step={0.1}
                        placeholder="Введите количество сахара..."
                        value={shiftInfo.sugarAmount}
                        onChange={(e) => {
                            setShiftInfo({ ...shiftInfo, sugarAmount: e.target.value })
                            setErrors({ ...errors, sugarAmount: "" })
                        }}
                    />
                    {errors.sugarAmount &&
                        <div className=" invalid-feedback">{errors.sugarAmount}</div>
                    }
                </div>
                <div>
                    <label>Количество цветного сахара (кг):</label>
                    <input className={`form-control ${errors.colorSugarAmount ? "is-invalid" : ""}`}
                        type="number"
                        min={0.1} max={999} step={0.1}
                        placeholder="Введите количество цветного сахара..."
                        value={shiftInfo.colorSugarAmount}
                        onChange={(e) => {
                            setShiftInfo({ ...shiftInfo, colorSugarAmount: e.target.value })
                            setErrors({ ...errors, colorSugarAmount: "" })
                        }}
                    />
                    {errors.colorSugarAmount &&
                        <div className=" invalid-feedback">{errors.colorSugarAmount}</div>
                    }
                </div>
                <div>
...
export default ActiveFoodShiftInfoUpdate